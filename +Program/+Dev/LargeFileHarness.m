classdef LargeFileHarness
    ...
    methods (Static)
        function report = run(source_file, output_dir)
            ...
            corrupt_file = fullfile(output_dir, 'corrupt-checkpoint.mat');
            Program.Dev.LargeFileHarness.delete_if_present(corrupt_file);
            copyfile(partial_file, corrupt_file);
            corrupt = load(corrupt_file, 'conversion_state');
            corrupt.conversion_state.completed_z = layout.output_z_count + 1;
            conversion_state = corrupt.conversion_state;
            save(corrupt_file, 'conversion_state', '-append');
            corrupt_checkpoint_rejected = ...
                ~DataHandling.Helpers.nwb.can_resume_image_conversion( ...
                    corrupt_file, checkpoint.conversion_state);
            assert(corrupt_checkpoint_rejected, ...
                'A corrupt checkpoint was incorrectly accepted for resume.');
            Program.Dev.LargeFileHarness.delete_if_present(corrupt_file);
