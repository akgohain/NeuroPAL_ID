classdef GUIHandling
    ...
    methods (Static)
            ...
            switch backend
                ...
                case "spotiflow_supervised"
                    specs = { ...
                        ...
                        struct('key', 'probability_threshold', 'label', 'Score cut', 'value', 0.185, 'limits', [0 1], 'integer', false, 'enabled', true), ...
            ...
            switch method
                case {"gat/transformer", "transformer", "anshita", "anshita gat", "gat"}
                    specs = { ...
                        ...
                        struct('key', 'checkpoint_path', 'label', 'Checkpoint', 'value', Methods.TransformerAutoId.defaultCheckpointPath(), 'limits', [], 'integer', false, 'enabled', true, 'path_kind', 'folder'), ...
        ...
        function run_modern_auto_detector(app, backend)
            ...
            try
                ...
                Program.GUIHandling.try_auto_detect_ui_step(app, 'clear UserNeuronIDsListBox.ItemsData', ...
                    @() set(app.UserNeuronIDsListBox, 'ItemsData', []));
                Program.GUIHandling.try_auto_detect_ui_step(app, 'clear UserNeuronIDsListBox.Value', ...
                    @() set(app.UserNeuronIDsListBox, 'Value', {}));
        ...
        function try_auto_detect_ui_step(app, label, step_fcn)
            Program.GUIHandling.auto_detect_log(app, 'UI step begin: %s.', label);
            try
                step_fcn();
                Program.GUIHandling.auto_detect_log(app, 'UI step complete: %s.', label);
            catch ME
                Program.GUIHandling.auto_detect_log(app, ...
                    'UI step warning: %s failed with %s: %s. Continuing.', ...
        ...
        function run_transformer_auto_id(app)
            ...
            try
                Methods.TransformerAutoId.run(app, ...
                    ...
                    'CheckpointPath', Program.GUIHandling.param_value(params, 'checkpoint_path', Methods.TransformerAutoId.defaultCheckpointPath()), ...
        ...
        function message = method_error_message(ME)
            ...
            switch char(string(ME.identifier))
                ...
                case 'Wrapper:SpotiflowCommandFailed'
                    message = sprintf(['Frozen Spotiflow v1 could not complete.\n\n%s\n\n' ...
                        'Verify the Spotiflow 0.6.5 environment and the three checkpoint files. ' ...
                        'Hash or version mismatches are blocked intentionally.'], ME.message);
        ...
        function message = transformer_match_success_message(app)
            ...
            try
                ...
                message = sprintf(['Auto-ID completed successfully.\n\n' ...
                    'Assigned IDs to %d of %d model predictions.\n' ...
                    'Unmatched neurons in the current view: %d'], ...
                ...
                if isfield(stats, 'max_distance_um') && isfinite(stats.max_distance_um)
                    message = sprintf('%s\nLargest assignment offset: %.2f um', ...
        ...
        function params = migrate_yolo_detect_params(params)
            if ~isstruct(params)
                params = struct();
            end
        ...
        function params = replace_missing_or_old_default(params, field_name, old_value, new_value)
            ...
            if isnumeric(value) && isscalar(value) && isfinite(value) && ...
                    abs(double(value) - old_value) < eps(old_value + 1)
                params.(field_name) = new_value;
            end
        end
